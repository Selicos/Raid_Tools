# Champion Log Generation Prompt for Lua

You are to generate a complete champion log for Lua in JSON format, using the following module templates as structure and guidance. Each module (0–13) is included below. For each, fill in the relevant information for Lua. Output a single JSON object with each module as a key (e.g., "overview", "skills", "synergy", etc.).
...
---
Instructions:
- Fill in each section for Lua using the module templates below.
