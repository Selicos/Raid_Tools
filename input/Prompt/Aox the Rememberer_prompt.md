...
You are to generate a complete champion log for Aox the Rememberer in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Aox the Rememberer. Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Aox the Rememberer using the template above.
