...
You are to generate a complete champion log for Alice the Wanderer in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Alice the Wanderer. Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Alice the Wanderer using the template above.
