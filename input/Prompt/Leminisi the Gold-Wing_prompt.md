...
You are to generate a complete champion log for Leminisi the Gold-Wing in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Leminisi the Gold-Wing. Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Leminisi the Gold-Wing using the template above.
