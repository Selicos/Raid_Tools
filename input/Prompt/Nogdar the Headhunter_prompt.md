...
You are to generate a complete champion log for Nogdar the Headhunter in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Nogdar the Headhunter. Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Nogdar the Headhunter using the template above.
