...
You are to generate a complete champion log for Quargan the Crowned in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Quargan the Crowned. Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Quargan the Crowned using the template above.
