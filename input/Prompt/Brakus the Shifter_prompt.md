# Champion Log Generation Prompt for Brakus the Shifter

You are to generate a complete champion log for Brakus the Shifter in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Brakus the Shifter. Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Brakus the Shifter using the template above.
