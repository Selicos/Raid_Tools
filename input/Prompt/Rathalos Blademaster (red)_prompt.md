...
You are to generate a complete champion log for Rathalos Blademaster (red) in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Rathalos Blademaster (red). Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Rathalos Blademaster (red) using the template above.
