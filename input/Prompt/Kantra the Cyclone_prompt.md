...
You are to generate a complete champion log for Kantra the Cyclone in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Kantra the Cyclone. Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Kantra the Cyclone using the template above.
