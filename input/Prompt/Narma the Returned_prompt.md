...
You are to generate a complete champion log for Narma the Returned in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Narma the Returned. Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Narma the Returned using the template above.
