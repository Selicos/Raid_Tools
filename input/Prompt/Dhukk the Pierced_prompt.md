...
You are to generate a complete champion log for Dhukk the Pierced in JSON format, using the following template as structure and guidance. Fill in all modules (0–20) for Dhukk the Pierced. Output a single JSON object with each module as a key.
...
---
Instructions:
- Fill in each section for Dhukk the Pierced using the template above.
