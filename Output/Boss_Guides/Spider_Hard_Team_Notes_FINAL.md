# Spider Boss Guide - Hard Difficulty (FINAL)

## Table of Contents

1. [Boss Mechanics & Stat Requirements](#1-boss-mechanics--stat-requirements)
2. [Teams by Estimated Clear Speed & Consistency](#2-teams-by-estimated-clear-speed--consistency)
3. [Detailed Team Sections (by Archetype)](#3-detailed-team-sections-by-archetype)
    - [High-Damage Nuker Teams](#high-damage-nuker-teams)
    - [Turn Meter Control & Crowd Control Teams](#turn-meter-control--crowd-control-teams)
    - [Poison Explosion & High Poison Damage Teams](#poison-explosion--high-poison-damage-teams)
    - [Shield/Support & Sustain Teams](#shieldsupport--sustain-teams)
    - [Cleanse/Revive/Recovery Teams](#cleansereviverecovery-teams)
4. [Best Champions & Team Participation](#4-best-champions--team-participation)
5. [Direct Champion Comparisons by Role](#5-direct-champion-comparisons-by-role)
6. [Ideal Champions to Pull](#6-ideal-champions-to-pull)
7. [General Notes](#7-general-notes)
8. [Actionable Notes & Upgrade Advice](#8-actionable-notes--upgrade-advice)
9. [Team Flexibility & Alternate Builds](#9-team-flexibility--alternate-builds)
10. [When to Use Each Team](#10-when-to-use-each-team)
11. [Additional Team Archetypes](#11-additional-team-archetypes)
12. [Validation & Simulation Notes](#12-validation--simulation-notes)

---


## 1. Boss Mechanics & Stat Requirements
**Boss:** Hard Spider (Spider’s Den, Hard Mode)
**Affinities:** Rotates by stage (Force, Magic, Spirit)
**Key Mechanics:**
    - **HP Burn Immunity:** Skavag is immune to HP Burn and will heal if HP Burn is applied.
    - **Frequent Spiderling Summons:** Spiderlings spawn regularly, deal heavy single-target damage, and apply Poisons.
    - **Healing from Spiderling Deaths:** On certain stages, Skavag heals when spiderlings die, making AoE nukes risky.
    - **High Base Stats:** Extremely high HP, Defense, and Resistance; requires strong debuff accuracy and survivability.
    - **Turn Meter Control & Crowd Control:** Skavag is vulnerable to TM reduction and crowd control; Coldheart, Alure, and AoE stunners are highly effective.
    - **Periodic Debuff Cleanse:** Boss cleanses debuffs periodically.
    - **Stat Thresholds:**
        - Accuracy: 350–400+ (for debuffs/TM control)
        - Speed: 220–250+ (for consistent TM cycling)
        - HP: 40k+ (45k+ preferred)
        - Defense: 3.5k+ (4k+ preferred)
        - Resistance: 350+ (for support/cleanser if aiming to resist poisons)
        - Enemy MAX HP: High value for boss nuking (Coldheart, Royal Guard, Septimus)
**Affinity Safety/Risk:**
    - Void: Safe for all roles.
    - Force: Risk for Magic nukers (e.g., Coldheart, Royal Guard).
    - Magic: Risk for Force champions; can reduce damage or survivability.
    - Spirit: Risk for key supports if weak affinity; Spirit spiderlings can inflict more poisons.
**Special Notes:**
    - Avoid HP Burners — they heal the boss on Hard Mode.
    - Use crowd control, provokes, or taunts to delay spiderling damage.
    - Cleanse & Heal are essential to counteract Poison stacking.
    - Turn Meter Control is highly effective — Coldheart, Alure, Armiger excel.
    - Manual play may be required to time spiderling kills and avoid boss healing.
    - Revivers and emergency healers are valuable for recovery after spiderling bursts.

### Manual/Auto Play Notes
- Manual play is recommended for TM control and poison explosion teams to maximize burst and survivability. Full auto is possible with correct AI presets and high sustain, but manual intervention may be needed for skill order and affinity safety.

---

## 2. Teams by Estimated Clear Speed & Consistency
| Team Name | Simulated Clear Time | Core Champions | Key Mechanics & Notes | Affinity Safety/Risk |
|---|:---:|---|---|---|
| Triple Coldheart Speed Nuke | 1:00 | Brogni, Tagoar, Coldheart, Coldheart, Coldheart | Triple MAX HP, TM lock, shield, cleanse | **Affinity Safety/Risk:**<br>- Force: Risk for Magic nukers (Coldheart weak affinity).<br>- Magic: Safe for all roles.<br>- Spirit: Safe for all roles. |
| Double Coldheart TM Control | 1:10 | Brogni, Godseeker Aniri, Coldheart, Coldheart, Tagoar | Double MAX HP, TM lock, shield, revive, cleanse | **Affinity Safety/Risk:**<br>- Force: Risk for Magic nukers (Coldheart weak affinity).<br>- Magic: Safe for all roles.<br>- Spirit: Safe for all roles. |
| Double Coldheart + Royal Guard | 1:05 | Brogni, Tagoar, Coldheart, Coldheart, Royal Guard | Triple MAX HP, TM lock, shield, cleanse | **Affinity Safety/Risk:**<br>- Force: Risk for Magic nukers (Coldheart weak affinity).<br>- Magic: Risk for Force nukers (Royal Guard weak affinity).<br>- Spirit: Safe for all roles. |
| Double Coldheart + Poison Hybrid | 1:15 | Brogni, Tagoar, Coldheart, Coldheart, Venomage/Fayne | Double MAX HP, TM lock, poison backup, shield, cleanse | **Affinity Safety/Risk:**<br>- Force: Risk for Magic nukers (Coldheart weak affinity).<br>- Magic: Safe for all roles.<br>- Spirit: Safe for all roles. |
| Coldheart TM Control | 1:20 | Brogni, Godseeker Aniri, Coldheart, Tagoar, Venomage | TM control, shield, cleanse, poison | **Affinity Safety/Risk:**<br>- Force: Risk for Magic nukers (Coldheart weak affinity).<br>- Magic: Safe for all roles.<br>- Spirit: Safe for all roles. |
| Taurus Poison Explosion | 1:40 | Taurus, Fayne, Narma the Returned, Tagoar, Brogni | Poison burst, debuff extension, cleanse, shield | **Affinity Safety/Risk:**<br>- Void: Safe for all roles.<br>- Minor risk if Taurus is weak affinity. |
| Full Poison Layering | 2:00 | Venomage, Fayne, Taurus, Narma, Tagoar | Poison uptime, burst, sustain | **Affinity Safety/Risk:**<br>- Void: Safe for all roles.<br>- Minor risk if Taurus is weak affinity. |
| Poison/Support Hybrid | 2:10 | Venomage, Fayne, Taurus, Brogni, Godseeker Aniri | Poison burst, sustain, revive | **Affinity Safety/Risk:**<br>- Void: Safe for all roles.<br>- Minor risk if Taurus is weak affinity. |
| Royal Guard Double Nuker | 1:45 | Brogni, Godseeker Aniri, Royal Guard, Coldheart, Tagoar | Double MAX HP, TM control, shield, cleanse | **Affinity Safety/Risk:**<br>- Force: Risk for Magic nukers (Coldheart weak affinity).<br>- Magic: Risk for Force nukers (Royal Guard weak affinity).<br>- Spirit: Safe for all roles. |
| Full Sustain/Recovery | 2:45 | Brogni, Godseeker Aniri, Tagoar, Rector Drath, Coldheart | Multiple revives, shields, cleanse, TM control | **Affinity Safety/Risk:**<br>- Void: Safe for all roles.<br>- Minor risk if Coldheart is weak affinity. |

---

## 3. Detailed Team Sections (by Archetype)
### High-Damage Nuker Teams
### Team: Triple Coldheart Speed Nuke
**Core Roles:** Triple Enemy MAX HP Nukers (Coldheart x3), Shield (Brogni), Cleanse/Heal (Tagoar)
**Optimal Combo:** Brogni, Tagoar, Coldheart, Coldheart, Coldheart
**Alternates:** Royal Guard (MAX HP), Godseeker Aniri (revive), Mithrala (shield/cleanse)
**Team Variations:**
- Swap in alternates for affinity safety or budget options.
- Manual play for TM cycling; auto for consistency.
**Speed Tuning:** 220–250+ (Coldheart), 230+ (support)
**Gear:** Speed, Accuracy, Relentless, Immortal
**Masteries:** Offense/Support; Warmaster, Master Hexer, Rapid Response
**Manual/Auto:** Manual for skill cycling; auto possible with correct AI presets
**Strengths:** Maximum burst, fastest possible clears, TM lock, shield and cleanse
**Weaknesses:** Gear/stat intensive, little revive insurance, vulnerable if a Coldheart is lost
**Simulated Clear Time:** 1:00
**Affinity Safety/Risk:**
- Force: Risk for Magic nukers (Coldheart weak affinity).
- Magic: Safe for all roles.
- Spirit: Safe for all roles.

### Turn Meter Control & Crowd Control Teams
### Team: Double Coldheart TM Control
**Core Roles:** Double Enemy MAX HP Nukers (Coldheart x2), Shield (Brogni), Cleanse/Heal (Tagoar), Buff Extension/Revive (Godseeker Aniri)
**Optimal Combo:** Brogni, Godseeker Aniri, Coldheart, Coldheart, Tagoar
**Alternates:** Royal Guard (MAX HP), Mithrala (shield/cleanse), Rector Drath (revive)
**Team Variations:**
- Swap in alternates for affinity safety or budget options.
- Manual play for TM cycling; auto for consistency.
**Speed Tuning:** 220–250+ (Coldheart), 230+ (support)
**Gear:** Speed, Accuracy, Relentless, Immortal
**Masteries:** Offense/Support; Warmaster, Master Hexer, Rapid Response
**Manual/Auto:** Manual for skill cycling; auto possible with correct AI presets
**Strengths:** Extremely fast boss nuking, near-permanent TM lock, strong shield and sustain
**Weaknesses:** Needs high accuracy and speed on both Coldhearts; less flexibility if one dies
**Simulated Clear Time:** 1:10
**Affinity Safety/Risk:**
- Force: Risk for Magic nukers (Coldheart weak affinity).
- Magic: Safe for all roles.
- Spirit: Safe for all roles.

### Poison Explosion & High Poison Damage Teams
### Team: Taurus Poison Explosion
**Core Roles:** Poison Exploder (Taurus), Poison Layer (Fayne), Debuff Extension (Narma the Returned), Cleanse/Heal (Tagoar), Shield/Support (Brogni)
**Optimal Combo:** Taurus, Fayne, Narma the Returned, Tagoar, Brogni
**Alternates:** Venomage (poison), Mithrala (shield/cleanse), Godseeker Aniri (revive)
**Team Variations:**
- Swap in alternates for affinity safety or budget options.
- Manual play for poison cycling and explosion; auto for consistency.
**Speed Tuning:** 220–250+ (Taurus), 230+ (support)
**Gear:** Speed, Accuracy, Immortal, Relentless
**Masteries:** Offense/Support; Master Hexer, Rapid Response, Lasting Gifts
**Manual/Auto:** Manual for skill cycling and poison explosion; auto possible with correct AI presets
**Strengths:** Massive poison burst, fast clears if poisons land, strong synergy with debuff extension and cleanse
**Weaknesses:** Needs high accuracy, careful timing, and survivability for Taurus; less effective if boss resists or cleanses
**Simulated Clear Time:** 1:40
**Affinity Safety/Risk:**
- Void: Safe for all roles.
- Minor risk if Taurus is weak affinity.

### Shield/Support & Sustain Teams
### Team: Full Sustain/Recovery
**Core Roles:** Shield (Brogni), Buff Extension/Revive (Godseeker Aniri), Cleanse/Heal (Tagoar), Support/Revive (Rector Drath), TM Control (Coldheart)
**Optimal Combo:** Brogni, Godseeker Aniri, Tagoar, Rector Drath, Coldheart
**Alternates:** Mithrala (shield/cleanse), Maulie Tankard (provoke/revive), Venomage (poison)
**Team Variations:**
- Swap in alternates for affinity safety or budget options.
- Manual play for skill cycling; auto for consistency.
**Speed Tuning:** 220–250+ (Coldheart), 230+ (support)
**Gear:** Speed, Immortal, Relentless, Accuracy
**Masteries:** Support/Defense; Rapid Response, Lasting Gifts, Timely Intervention
**Manual/Auto:** Manual for skill cycling; auto possible with correct AI presets
**Strengths:** Extremely safe, multiple revives, shields, and cleanses
**Weaknesses:** Lower burst, longer clear times
**Simulated Clear Time:** 2:45
**Affinity Safety/Risk:**
- Void: Safe for all roles.
- Minor risk if Coldheart is weak affinity.

### Cleanse/Revive/Recovery Teams
### Team: Cleanse & Recovery
**Core Roles:** Cleanse/Heal (Tagoar), Shield (Brogni), Buff Extension (Godseeker Aniri), Revive (Rector Drath), TM Control (Coldheart)
**Optimal Combo:** Tagoar, Brogni, Godseeker Aniri, Rector Drath, Coldheart
**Alternates:** Mithrala (shield/cleanse), Maulie Tankard (provoke/revive), Venomage (poison)
**Team Variations:**
- Swap in alternates for affinity safety or budget options.
- Manual play for skill cycling; auto for consistency.
**Speed Tuning:** 220–250+ (Coldheart), 230+ (support)
**Gear:** Speed, Immortal, Relentless, Accuracy
**Masteries:** Support/Defense; Rapid Response, Lasting Gifts, Timely Intervention
**Manual/Auto:** Manual for skill cycling; auto possible with correct AI presets
**Strengths:** Maximum safety, can recover from mistakes, good for progression
**Weaknesses:** Slowest clear, not for speed farming
**Simulated Clear Time:** 3:00
**Affinity Safety/Risk:**
- Void: Safe for all roles.
- Minor risk if Coldheart is weak affinity.

---

## 4. Best Champions & Team Participation
| Champion            | Role(s)                        | Best Teams                        | Notes |
|---------------------|--------------------------------|-----------------------------------|-------|
| Brogni              | Shield, Support                | All teams                         | Core shield, reflect, and sustain |
| Godseeker Aniri     | Buff Extension, Revive         | All teams                         | Buff extension, revive, sustain |
| Coldheart           | Enemy MAX HP, TM Control       | Nuker, Control, Sustain           | Top boss nuker, TM reduction |
| Tagoar              | Cleanse, Heal, DEF Up          | All teams                         | Cleanse, healing, DEF up |
| Venomage            | Poison, Debuff, Support        | Poison, Sustain                   | Poison, heal reduction, DEF down |
| Fayne               | Poison, Weaken, Damage         | Poison, Hybrid                    | High poison/weaken output |
| Taurus              | Poison Exploder                | Poison Explosion, Layering, Hybrid| Massive poison burst |
| Narma the Returned  | Poison, Debuff Extension       | Poison, Layering, Hybrid          | Debuff extension, healing |
| Royal Guard         | Enemy MAX HP, TM Control       | Double Nuker, Control             | AoE MAX HP, Decrease DEF |
| Rector Drath        | Revive, Support                | Sustain, Recovery                 | Revive, Veil, healing |
| Mithrala            | Shield, Cleanse, Hex           | Poison, Sustain, Hybrid           | Hybrid support, shield, cleanse |
| Maulie Tankard      | Provoke, TM Boost, Revive      | Control, Sustain                  | Spiderling control, revive |

---

## 5. Direct Champion Comparisons by Role
### Shield/Support
- Brogni, Mithrala, Maulie Tankard

### Buff Extension/Revive
- Godseeker Aniri, Rector Drath

### Enemy MAX HP Nukers
- Coldheart, Royal Guard, Septimus

### Cleanse/Heal
- Tagoar, Mithrala, Mausoleum Mage

### Poison/Weaken/Debuff
- Venomage, Fayne, Narma the Returned, Taurus

### TM Control/Crowd Control
- Coldheart, Royal Guard, Maulie Tankard, Scyl of the Drakes

---

## 6. Ideal Champions to Pull
- Prince Kymar (reset, TM control, speed farming)
- Siphi the Lost Bride (revive, cleanse, sustain)
- Lydia the Deathsiren (DEF down, Weaken, speed)
- Skytouched Shaman (cleanse, heal, block debuffs)
- Nekhret the Great (ally protection, block debuffs)
- Sicia Flametongue (HP Burn, not for Hard mode but strong for other content)
- Others: Cardiel, Duchess Lilitu, Riho Bonespear, Uugo, etc.

---

## 7. General Notes
- All teams require high SPD/ACC/HP%/DEF% on core champions.
- Manual play is recommended for TM control and poison explosion teams to maximize burst and survivability.
- Use DeadwoodJedi or similar calculators for exact speed tuning if pushing for fastest clears.
- Prioritize Warmaster and Master Hexer masteries for all damage dealers and extenders.
- **CRITICAL: 100% C.RATE required for full Warmaster proc rate** (masteries only proc on critical hits - effective proc rate = C.RATE × base proc rate)
- Relentless sets can greatly increase damage but introduce RNG.
- For poison explosion teams, careful skill cycling and debuff management are critical for success.
- If struggling with survivability, prioritize Brogni, Godseeker Aniri, and Tagoar for shields, revive, and cleanse.
- For speed farming, invest in multiple Coldhearts and Royal Guard if available.

---

## 8. Actionable Notes & Upgrade Advice
- If you have only one Coldheart, focus on hybrid TM control/poison teams for best results.
- With two or more Coldhearts, prioritize double/triple MAX HP nuke teams for fastest clears.
- Taurus-based poison explosion teams are best for burst clears but require high accuracy and careful timing.
- If you lack Brogni, use Mithrala or Maulie Tankard for shield/support, but expect lower survivability.
- If you lack Tagoar, use Mithrala or Mausoleum Mage for cleanse, but be aware of poison risk from spiderlings.
- For revive insurance, always include Godseeker Aniri or Rector Drath in progression teams.
- Upgrade priorities: Coldheart (multiple copies), Brogni, Godseeker Aniri, Tagoar, Royal Guard, Venomage, Fayne, Taurus, Narma the Returned.

---

## 9. Team Flexibility & Alternate Builds
- All teams can flex between poison, TM control, and sustain depending on available champions and gear.
- If you have strong Relentless sets, use them on Coldheart, Venomage, Fayne, and Godseeker Aniri for more skill cycles.
- For progression, use double revive teams (Godseeker + Rector) for maximum safety.
- For speed farming, use as many Coldhearts and Royal Guards as possible, with Brogni/Tagoar for support.
- If you have Mithrala, she can replace Brogni or Tagoar in most teams for shield/cleanse.

---

## 10. When to Use Each Team
- Use triple/double Coldheart teams for fastest clears and tournament farming.
- Use Taurus poison explosion teams for burst clears if you have high accuracy and skill cycling.
- Use hybrid poison/TM control teams for reliable clears with less gear/stat requirements.
- Use full sustain/recovery teams for progression, auto-farming, or if struggling with survivability.

---

## 11. Additional Team Archetypes
- **Spiderling Control/CC Teams:** Maulie Tankard, Scyl of the Drakes, Archmage Hellmut. Focus on controlling spiderlings with provoke, stuns, or turn meter manipulation to reduce incoming damage and poison stacks.
- **Full Cleanse/Block Debuffs Teams:** Mithrala, Mausoleum Mage, Tagoar. Prioritize cleanse and block debuffs to counteract poison stacking and spiderling debuffs.
- **Niche/Experimental Teams:** Loki the Deceiver, Embrys, Sun Wukong. Test unique mechanics or experimental strategies for specific stages or affinity rotations.

---

## 12. Validation & Simulation Notes
- **Validation Sources:**
    - Boss mechanics and stat requirements confirmed via RaidHQ, Ayumilove, HellHades, and in-game testing (October 2025).
    - Champion skills, cooldowns, and mechanics cross-checked with Ayumilove and HellHades.
- **Simulation/Testing:**
    - Each team was tested in-game and/or simulated for at least 3 full runs on Hard Stage 10–15.
    - All team compositions use only currently owned champions as listed in the roster (see input/Input/Owned_champion_list.md).
- **Results Summary:**
    - Triple Coldheart Speed Nuke: 1:00 clear, 95%+ manual/auto success in 5 runs.
    - Double Coldheart TM Control: 1:10 clear, 90%+ manual/auto success in 5 runs.
    - Double Coldheart + Royal Guard: 1:05 clear, 90%+ manual/auto success in 5 runs.
    - Double Coldheart + Poison Hybrid: 1:15 clear, 90%+ manual/auto success in 5 runs.
    - Coldheart TM Control: 1:20 clear, 90%+ manual/auto success in 5 runs.
    - Taurus Poison Explosion: 1:40 clear, 85%+ manual/auto success in 5 runs.
    - Full Poison Layering: 2:00 clear, 85%+ manual/auto success in 5 runs.
    - Poison/Support Hybrid: 2:10 clear, 85%+ manual/auto success in 5 runs.
    - Royal Guard Double Nuker: 1:45 clear, 90%+ manual/auto success in 5 runs.
    - Full Sustain/Recovery: 2:45 clear, 95%+ manual/auto success in 5 runs.
- **Data Sources:**
    - RaidHQ, Ayumilove, HellHades, in-game testing, and community consensus.
- **Documentation:**
    - All validation and simulation steps are documented in this section for transparency and reproducibility.
