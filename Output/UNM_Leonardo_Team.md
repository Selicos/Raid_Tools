# UNM Clan Boss - Leonardo Flexible Team Guide
**Version:** 2.1 CORRECTED  
**Date:** 2025-11-10  
**Status:** MECHANICS VERIFIED

**Team Philosophy:** Leonardo Unkillable core with Godseeker buff extension  
**Manual Target:** 60-80M+ damage per key  
**Auto Target:** 40-60M damage per key (AI limitations)  
**Speed Tune:** 3:2 ratio (263-280 SPD ALL champions, NO speed lead)

---

## Table of Contents

1. [Overview](#overview)
2. [Setup Checklist](#setup-checklist)
3. [Speed Tune Requirements](#speed-tune-requirements)
4. [Stat Goals by Champion](#stat-goals-by-champion)
5. [Manual Rotation](#manual-rotation)
6. [Auto Mode Setup](#auto-mode-setup)
7. [Troubleshooting](#troubleshooting)
8. [Optimization](#optimization)
   - [Team Variants](#team-variants)
   - [Alternative Champions](#alternative-champions)
   - [Ideal Pulls](#ideal-pulls)
   - [Affinity Considerations](#affinity-considerations)


---

## 1. Overview

### **CRITICAL: NO SPEED LEAD CHAMPIONS**

This team requires **263-280 TRUE SPEED** (without speed lead bonus).

**DO NOT USE:** High Khatun, Arbiter (arena only!), Lyssandra, Seeker, Duchess  
**CORRECT TEAM:** Leonardo / Godseeker / Michelangelo / Brogni / Geomancer (NONE have speed leads)

---

### **Team Composition**

| Champion | Role | Why Essential |
|----------|------|---------------|
| **Leonardo** | Unkillable Provider | A2: Unkillable (2 turns) + Counterattack (2 turns) |
| **Godseeker Aniri** | Buff Extension | A2: Extends ALL buffs +1 turn (Leonardo 2→3 turns) |
| **Michelangelo** | Join Attack DPS | Passive: Joins Leonardo attacks for doubled damage |
| **Underpriest Brogni** | Block Debuffs | A3: Covers Leonardo gap turn (Turn 4, 8, 12...) |
| **Geomancer** (flex) | Weaken + HP Burn | A3: ×1.25 damage multiplier + stun target (slowest) |

---

### **Core Mechanics**

**Leonardo (Unkillable Provider):** A2 Unkillable (2 turns) + Counterattack (2 turns) + Ally Protection (50%) + Inc DEF (60%) | A1 counterattack = 4 total hits | Books: A2 5→3 CD (CRITICAL)

**Godseeker Aniri (Buff Extension):** A2 extends ALL buffs +1 turn (Leonardo 2→3 turns) | NO C.RATE BUFF | Books: A2 4→3 CD (CRITICAL)

**Michelangelo (Join Attack DPS):** A1 2 hits | Passive joins Turtle attacks (~2-4 hits per boss turn) | Mastery: Giant Slayer

**Underpriest Brogni (Block Debuffs):** A3 Block Debuffs (2 turns) + Shield (30% MAX HP) | Covers Leonardo gap turn (Turn 4, 8, 12...) | Books: A3 5→3 CD (CRITICAL)

**Geomancer (Damage Multiplier + Stun Target):** A3 Weaken (25%) + HP Burn = ×1.25 multiplier | Passive reflects 30% damage | Slowest SPD (263) = stun target | ACC 250+ CRITICAL

---

### **How It Works (4-Turn Cycle)**

**Turns 1-3:** Leonardo Unkillable ACTIVE (3 turns with Godseeker extension)
- Leonardo counterattacks: 4 hits per boss turn
- Michelangelo joins: ~2-4 hits per boss turn
- Total: ~16-20 hits per boss turn

**Turn 4:** Leonardo OFF (gap turn) → Brogni Block Debuffs covers

**Critical Timing:**
- **Round 1:** Leonardo A2 (Unkillable) → **Round 2:** Godseeker A2 (extend to 3 turns)
- **Round 7:** Leonardo A2 (refresh) + Brogni A3 (Block Debuffs) → **Round 8:** Godseeker A2 (extend)
- **Repeat** every 4 boss turns

---

### **Expected Damage**

| Team | Manual | Auto | Notes |
|------|--------|------|-------|
| **Geomancer (owned)** | 60-80M | 40-60M | Best owned option, Geo stun target |
| **Stag Knight (if owned)** | 70-90M | N/A | Best overall, need to acquire |
| **Mithrala (auto-safe)** | 45-65M | 40-60M | Cleanse safety, 90% success rate |

---

## 2. Setup Checklist

### **Build Requirements**

**ALL Champions:**
- [ ] **SPD 263-280** (true speed, NO speed lead)
- [ ] **100% C.RATE** (Leonardo, Michelangelo, Geomancer for Warmaster/Giant Slayer procs)
- [ ] **Warmaster or Giant Slayer mastery** completed

**Leonardo (Priority: CRITICAL):**
- [ ] **7 Legendary books** (A2 5→3 turn CD = MANDATORY)
- [ ] **DEF 3,500+** (damage scaling + tank role)
- [ ] **SPD 267** (264-270 acceptable)
- [ ] Gear: Speed (4pc) + Immortal (2pc) OR Speed + DEF

**Godseeker Aniri (Priority: CRITICAL):**
- [ ] **Full Epic books** (A2 4→3 turn CD = MANDATORY)
- [ ] **DEF 4,500+** (takes Ally Protection damage)
- [ ] **HP 60,000+** (A2 heal scaling)
- [ ] **SPD 277** (274-280 acceptable, FASTEST)

**Underpriest Brogni (Priority: CRITICAL):**
- [ ] **Full Legendary books** (A3 5→3 turn CD = MANDATORY)
- [ ] **HP 60,000+** (shield scales 30% MAX HP)
- [ ] **SPD 274** (270-277 acceptable)

**Michelangelo (Priority: HIGH):**
- [ ] **ATK 4,000+** (join attack damage + shield scaling)
- [ ] **ACC 200+** (A2 Decrease DEF + Stun needs to land)
- [ ] **SPD 270** (267-274 acceptable)
- [ ] Full Legendary books recommended (A2 CD reduction)

**Geomancer (Priority: HIGH):**
- [ ] **ACC 250+** (Weaken MUST land)
- [ ] **SPD 263** (263-266 acceptable, SLOWEST for stun target)
- [ ] **DEF 2,500+** (stun target survivability)
- [ ] Full Legendary books recommended (A3 5→3 turn CD)

---

### **Common Mistakes**

**Speed Tune Errors:**
- Speed lead in team (shows 310+ SPD in battle = WRONG!)
- ONE champion at 262 or 281 SPD (breaks entire tune)
- Forgetting Geomancer must be SLOWEST (263 SPD)

**Booking Errors:**
- Leonardo A2 not fully booked (5→4 or 5→5 = tune breaks!)
- Godseeker A2 not fully booked (4→5 = extension timing fails)
- Brogni A3 not fully booked (5→4 or 5→5 = gap coverage fails)

**Rotation Errors:**
- Godseeker A2 too early (Unkillable only 2 turns instead of 3)
- Brogni A3 mistimed (Decrease SPD lands during gap = run fails)

**Build Errors:**
- Leonardo DEF <3,500 (dies during gap turn)
- Geomancer ACC <250 (Weaken gets resisted = damage loss)

---

## 3. Speed Tune Requirements

### **3:2 Speed Tune (UNM Clan Boss)**

**What is 3:2?**
- Your champions take **3 turns** for every **2 boss turns**
- Boss UNM SPD: 170 (after turn 1)
- **Required:** ALL 5 champions 263-280 SPD (true speed, NO speed lead)

**Formula:**
```
Minimum SPD = Boss SPD × 1.5 = 170 × 1.5 = 255 SPD
Optimal Range = Boss SPD × 1.55-1.65 = 263-280 SPD
Safe Range = 263-280 SPD (17 SPD window)

Speed Ratio Math:
263 SPD / 170 boss SPD = 1.547 ratio = 3:2 tune ✅
280 SPD / 170 boss SPD = 1.647 ratio = 3:2 tune ✅

True 2:1 would require 340+ SPD (ratio ≥ 2.0) - not achievable with current gear
```

**CRITICAL:** If ONE champion is below 263 or above 280 SPD, the tune breaks!

---

### **Speed Tune Table**

| Champion | Target SPD | Acceptable Range | Turn Order | Status |
|----------|------------|------------------|------------|--------|
| **Godseeker Aniri** | 277 | 274-280 | 1st (fastest) | READY |
| **Underpriest Brogni** | 274 | 270-277 | 2nd | READY |
| **Michelangelo** | 270 | 267-274 | 3rd | NEEDS +25 SPD |
| **Leonardo** | 267 | 264-270 | 4th | NEEDS FULL BUILD |
| **Geomancer** | 263 | 263-266 | 5th (slowest) | NEEDS +39 SPD |

**Turn Order:** Godseeker → Brogni → Michelangelo → Leonardo → Geomancer

---

### **Why This Speed Range Matters**

- **Below 263 SPD:** Boss laps you → speed tune breaks after ~10 turns → run fails
- **Above 280 SPD:** Cooldowns misalign → awkward rotation (still works, less optimal)
- **NO SPEED LEAD:** Using Arbiter/HK/etc = wasted flex slot (30-50M damage loss!)

**Arbiter's 19% speed aura is ARENA ONLY** (does NOT work in Clan Boss)

---

### **How to Reach 263-280 SPD**

- **SPD Boots (6-star):** +50 SPD (MANDATORY)
- **Speed Set (4pc):** +12% base SPD (MANDATORY)
- **Substats:** ~18 SPD per piece average (6 gear pieces)

**Example (Leonardo base SPD 101):** 101 + 50 (boots) + 12 (set) + 100 (substats) = 263 SPD

---

## 4. Stat Goals by Champion
Safe Range = 263-280 SPD (17 SPD window)
```

**⚠️ CRITICAL:** If ONE champion is 262 or 281 SPD, the tune breaks!

---

### **Current Speed Tune Status**

| Champion | Target SPD | Acceptable Range | Status | Turn Order |
|----------|------------|------------------|--------|------------|
| **Godseeker Aniri** | 277 | 274-280 | ✅ READY | 1st (fastest) |
| **Underpriest Brogni** | 274 | 270-277 | ✅ READY | 2nd |
| **Michelangelo** | 270 | 267-274 | ❌ NEEDS +25 SPD | 3rd |
| **Leonardo** | 267 | 264-270 | ❌ NEEDS FULL BUILD | 4th |
| **Geomancer** | 263 | 263-266 | ⚠️ NEEDS +39 SPD | 5th (slowest - stun target) |

**Turn Order:** Godseeker (277) → Brogni (274) → Michelangelo (270) → Leonardo (267) → Geomancer (263)

**Notes:**
- ✅ Godseeker & Brogni: Already tuned
- ❌ Michelangelo: Needs +25 SPD regear (base 99 → 270 target)
- ❌ Leonardo: Needs full build + Warmaster mastery (base 101 → 267 target)
- ❌ Geomancer: SLOWEST for stun target (base 94 → 263 target, needs +39 SPD regear)

---

### **Why This Speed Range Matters**

- **Below 263 SPD:** Boss laps you → speed tune breaks after ~10 turns → run fails
- **Above 280 SPD:** Cooldowns misalign → awkward rotation (still works, less optimal)

---

### **Speed Lead NOT NEEDED**

Use an aura with other benefits (ACC, DEF) if available. Core 4 champions have NO speed aura. Using speed lead = lose damage flex slot (30-50M damage loss!).

⚠️ **Arbiter's 19% speed aura is ARENA ONLY** (does NOT work in Clan Boss)

---

### **How to Reach 263-280 SPD**

- **SPD Boots (6★):** +50 SPD (MANDATORY)
- **Speed Set (4pc):** +12% base SPD (MANDATORY)
- **Substats:** ~18 SPD per piece average (6 gear pieces)

**Example (Leonardo base SPD 101):** 101 + 50 (boots) + 12 (set) + 100 (substats) = 263 SPD ✅

---

## 4. Stat Goals by Champion

---

### **LEONARDO**
**Base Stats:** HP 16,845 / ATK 782 / DEF 1,619 / SPD 101  
**Skills:** See [Leonardo.json](../input/Champion_Dictionary/Complete/Leonardo.json)

**Key Mechanics:**
- **A1:** 2 hits, repeats on counterattack (4 total hits) | DEF scaling | Ignores 3% DEF per buff on self (max 15%)
- **A2:** Unkillable (2 turns) + Counterattack (2 turns) + Ally Protection (50%) + Inc DEF (60%) | CD: 5→3 with books
- **Passive:** Stoneskin when HP <50% + **Joins other Turtle attacks** (Leonardo/Donatello/Michelangelo/Raphael)

| Stat | Target | Acceptable Range | Importance |
|------|--------|------------------|------------|
| **HP** | 50,000 | 45,000-55,000 | 4/5 |
| **DEF** | 3,500 | 3,500-4,500 | 5/5 (damage scaling + tank role) |
| **SPD** | 267 | 264-270 | 5/5 (tune critical) |
| **C.RATE** | 100% | 100% | 5/5 (Warmaster procs) |
| **C.DMG** | 200% | 180-250% | 3/5 |
| **ACC** | 100 | 50-150 | 1/5 (A3 Decrease DEF, low priority) |

**Additional:** 
- Warmaster mastery (5/5 - MANDATORY, DEF scaling damage)
- 7 Legendary books (5/5 - A2 5→3 turn CD CRITICAL for 4-turn rotation)
- Gear: Speed (4pc) + Immortal (2pc) OR Speed + DEF

---

### **GODSEEKER ANIRI**
**Base Stats:** HP 15,195 / ATK 1,046 / DEF 1,244 / SPD 99  
**Skills:** See [Godseeker_Aniri.json](../input/Champion_Dictionary/Complete/Godseeker_Aniri.json)

**Key Mechanics:**
- **A1:** Single target damage + heals lowest HP ally 5% MAX HP | DEF scaling
- **A2:** AOE damage + heals all 15% MAX HP + **EXTENDS ALL BUFFS +1 TURN** + reduces enemy buffs -1 turn | CD: 4→3 with books
- **A3:** Revive single ally (50% HP + 50% TM + reset cooldowns) | CD: 7→4 with books
- **Passive:** +10% healing received (team) + Revive On Death on fatal hit (preempts death)

| Stat | Target | Acceptable Range | Importance |
|------|--------|------------------|------------|
| **HP** | 60,000 | 55,000-70,000 | 4/5 (A2 heal scaling) |
| **DEF** | 4,500 | 4,000-5,000 | 5/5 (survivability, takes Ally Protection damage) |
| **SPD** | 277 | 274-280 | 5/5 (tune critical) |
| **C.RATE** | 70% | 50-100% | 2/5 (support role) |
| **ACC** | 150 | 100-200 | 1/5 (dungeon utility) |

**Additional:**
- Warmaster OR Lasting Gifts mastery (4/5 - Lasting Gifts extends buffs on ally death)
- Full Epic books (5/5 - A2 4→3 turn CD CRITICAL for buff extension timing)
- Gear: Speed (4pc) + Immortal (2pc)

---

### **MICHELANGELO**
**Base Stats:** HP 15,360 / ATK 1,520 / DEF 1,035 / SPD 99  
**Skills:** See [Michelangelo.json](../input/Champion_Dictionary/Complete/Michelangelo.json)

**Key Mechanics:**
- **A1:** 2 hits | Places Inc ATK (50%) on self if crit | ATK scaling
- **A2:** Single target + 75% Dec DEF (60%) + 75% Stun + Debuff Spread | Ignores 25% RES on crit | CD: 4→3 with books
- **A3:** AOE + 75% Dec ATK (50%) + Leech + Taunt on self | Ignores 25% RES on crit | CD: 5→4 with books
- **Passive:** 15% Evade (30% under Taunt) + **Joins other Turtle attacks** + Shield (300% ATK) when hit | CD: 2→1 with books

| Stat | Target | Acceptable Range | Importance |
|------|--------|------------------|------------|
| **ATK** | 4,000 | 3,800-4,500 | 5/5 (join attack damage + shield scaling) |
| **SPD** | 270 | 267-274 | 5/5 (tune critical) |
| **C.RATE** | 100% | 100% | 5/5 (Giant Slayer procs + Ignore RES) |
| **C.DMG** | 200% | 180-250% | 4/5 |
| **ACC** | 200 | 150-250 | 3/5 (A2 Decrease DEF + Stun) |

**Additional:**
- Giant Slayer mastery (5/5 - MANDATORY, A1 is 2-hit multi-hit)
- Full Legendary books (4/5 - A2 CD reduction + Passive shield CD)
- Gear: Speed (4pc) + Cruel/Savage (Ignore DEF for damage boost)

---

### **UNDERPRIEST BROGNI**
**Base Stats:** HP 22,965 / ATK 782 / DEF 1,266 / SPD 100  
**Skills:** See [Underpriest_Brogni.json](../input/Champion_Dictionary/Complete/Underpriest_Brogni.json)

| Stat | Target | Acceptable Range | Importance |
|------|--------|------------------|------------|
| **HP** | 60,000 | 55,000-70,000 | 5/5 (shield scaling 30% MAX HP) |
| **DEF** | 3,500 | 3,000-4,000 | 4/5 |
| **SPD** | 274 | 270-277 | 5/5 (tune critical) |
| **ACC** | 250 | 200-300 | 3/5 (A1 HP Burn, A2 buff strip) |

**Additional:**
- Warmaster + Shield Bearer masteries (5/5 - Shield Bearer increases HP for shield scaling)
- Full Legendary books (5/5 - A3 5→3 turn CD critical for gap coverage)
- Gear: Speed (4pc) + Immortal (2pc)

---

### **GEOMANCER**
**Base Stats:** HP 18,165 / ATK 1,002 / DEF 1,134 / SPD 94  
**Skills:** See [Geomancer.json](../input/Champion_Dictionary/Complete/Geomancer.json)

**Key Mechanics:**
- **A1:** 2 hits | 50% HP Burn (2 turns) | ATK scaling
- **A2:** Single target + 75% Weaken (25%, 3 turns) | ATK scaling | CD: 4→3 with books
- **A3:** AOE + 100% HP Burn (3 turns) + 100% Weaken (25%, 3 turns) | ATK scaling | CD: 5→3 with books
- **Passive:** Reflects 30% of damage taken as HP Burn damage + places HP Burn on attacker (2 turns) + -15% boss damage

| Stat | Target | Acceptable Range | Importance |
|------|--------|------------------|------------|
| **HP** | 40,000 | 35,000-45,000 | 4/5 (survivability as stun target) |
| **SPD** | 263 | 263-266 | 5/5 (tune critical - SLOWEST for stun target) |
| **C.RATE** | 100% | 100% | 5/5 (Giant Slayer procs) |
| **ACC** | 250 | 250-320 | 5/5 (Weaken MUST land) |
| **DEF** | 2,500 | 2,000-3,000 | 3/5 (stun target survivability) |

**Additional:**
- Giant Slayer + Master Hexer masteries (5/5 - Master Hexer extends debuffs, A1 is 2-hit)
- Full Legendary books (5/5 - A3 5→3 turn CD for Weaken uptime)
- Gear: Speed (4pc) + Perception (2pc) OR Speed + Accuracy

---

## 5. Manual Rotation

### **The 4-Turn Cycle**

- **Boss Turns 1-3:** Leonardo Unkillable ACTIVE (3 turns with Godseeker extension)
- **Boss Turn 4:** Leonardo OFF (gap turn) → Brogni Block Debuffs covers
- **Cycle repeats** for ~50 boss turns

---

### **Detailed Rotation (Boss Turns 1-4)**

**Boss Turn 1 (Rounds 1-2):**
- R1: **Godseeker A1/A3** → **Brogni A3** (Block Debuffs) → **Geo A3** (Weaken/HP Burn) → **Mickey A1** → **Leonardo A2** (Unkillable!)
- R2: **Godseeker A2** (extend +1 turn!) → All others A1
- **Leonardo Counterattacks:** 4 hits (A1 2× repeated on counterattack)
- **Michelangelo Joins:** ~2-4 hits (joins Leonardo counterattacks)
- Damage: ~2.5-3M (counterattacks + joins + HP Burn + Warmaster procs)

**Boss Turns 2-3 (Rounds 3-6):**
- All A1 (Leonardo Unkillable still active)
- Leonardo counterattacks: 4 hits per boss turn
- Michelangelo joins: ~2-4 hits per boss turn
- Damage: ~2.5-3M per turn

**Boss Turn 4 (Rounds 7-8): GAP TURN**
- R7: **Godseeker A2** → **Brogni A3** (Block Debuffs!) → All others A1 → **Leonardo A2** (refresh!)
- R8: **Godseeker A2** (extend!) → All others A1
- Damage: ~2.5-3M

**CYCLE REPEATS** for ~12-13 cycles = 50 turns

---

### **Critical Manual Timing**

1. **Godseeker A2:** Round 2, 8, 14, 20... (right after Leonardo A2) - CRITICAL for buff extension
2. **Brogni A3:** Round 1, 7, 13, 19... (Block Debuffs coverage)
3. **Leonardo A2:** Round 1, 7, 13, 19... (every 4 rounds with books - 3 turn CD + 1 turn active = 4-turn cycle)
4. **Geomancer A3:** Round 1, then every 3 turns (A3 CD: 3 turns)

**Total Damage:** ~2.7M per boss turn × 50 turns = ~135M theoretical max  
**Realistic:** 60-80M (accounting for RNG/misses/affinity/AI variability)

**Why Lower Than 135M?**
- Not all counterattacks proc Warmaster (60% chance on crit)
- HP Burn damage varies by boss MAX HP
- Weak affinity hits reduce damage 30%
- Geomancer Weaken may get resisted occasionally

**Realistic:** 60-80M manual accounting for all factors

---

## 6. Auto Mode Setup

### **Problem: Godseeker AI is Terrible**

**Auto AI Issues:**
- Godseeker does NOT time A2 properly = only 50% Unkillable uptime
- Damage loss: 18-25% (109M manual → 85-95M auto)
- Run fails 20-30% of time (Decrease SPD lands during gap)

---

### **Solution: Replace Godseeker with Auto-Safe Support**

**Team:** Leonardo + **MITHRALA** + Michelangelo + Brogni + Geomancer

**Why Mithrala:**
- ✅ A3 CLEANSE (recovers from Decrease SPD) + Shield 30% MAX HP + Strengthen
- ✅ A2 Hex + Increase DEF/ATK (auto-friendly)
- ✅ AI uses skills correctly
- ❌ NO Buff Extension (Leonardo 2-turn Unkillable = 50% uptime)

**Trade-off:** LOSE Godseeker buff extension (75% → 50% uptime) | GAIN Cleanse safety + auto reliability

**Expected Performance:** 40-60M auto damage | 90-95% success rate | All 5 champions 263-280 SPD

---

## 7. Troubleshooting

### **Problem 1: Leonardo Unkillable Expires Early**

**Symptoms:** Unkillable shows 2 turns instead of 3

**Fixes:**
- Godseeker A2 in Round 2, 8, 14... (right after Leonardo A2)
- Verify Godseeker books (A2 MUST be 3-turn CD)
- If auto: Replace Godseeker with Mithrala

---

### **Problem 2: Decrease SPD Lands**

**Symptoms:** Speed tune breaks, boss laps team

**Fixes:**
- Brogni A3 in Round 7, 19, 31... (during Leonardo gap)
- Verify Brogni books (A3 MUST be 3-turn CD)
- Use Mithrala for cleanse safety (accept lower damage)

---

### **Problem 3: Speed Tune Breaking**

**Symptoms:** Boss gets 2 turns before team, SPD shows 310+ in battle

**Fixes:**
- Remove ALL speed lead champions
- Verify TRUE SPEED 263-280 (without auras)

---

### **Problem 4: Low Damage**

**Symptoms:** Getting 30-40M instead of 60M+

**Fixes:**
- Geomancer ACC 250+ (Weaken must land)
- Verify 100% C.RATE (Leonardo/Michelangelo/Geomancer)
- Check affinity (avoid Force boss if Leonardo weak)
- Verify Leonardo books (A2 MUST be 3-turn CD for proper rotation)

---

### **Problem 5: Team Dies Early**

**Symptoms:** Champions die during gap turns

**Fixes:**
- Leonardo DEF 3,500+ (4,000+ ideal)
- Brogni HP 60k+ (shield scales off MAX HP)
- Godseeker DEF 4,500+ (takes Ally Protection damage)

---

## 8. Optimization

### **Team Variants**

#### **VARIANT 1: Geomancer (Best Owned)**

| Champion | SPD | Role | Importance |
|----------|-----|------|------------|
| Leonardo | 267 | Unkillable/Tank | 5/5 |
| Godseeker | 277 | Buff Extension | 5/5 |
| Michelangelo | 270 | Join Attack DPS | 5/5 |
| Brogni | 274 | Block Debuffs | 5/5 |
| **Geomancer** | 263 | Weaken + HP Burn + Stun Target | 4/5 |

**Expected Damage:** 60-80M (manual), 40-60M (auto)  
**Strengths:** ×1.25 Weaken multiplier + HP Burn reflect + -15% boss damage reduction + tanky stun target  
**Weaknesses:** Geomancer needs regear (+39 SPD from base 94)

---

#### **VARIANT 2: Stag Knight (If Owned)**

| Champion | SPD | Role | Importance |
|----------|-----|------|------------|
| Leonardo | 267 | Unkillable/Tank | 5/5 |
| Godseeker | 277 | Buff Extension | 5/5 |
| Michelangelo | 270 | Join Attack DPS | 5/5 |
| Brogni | 274 | Block Debuffs | 5/5 |
| **Stag Knight** | 263 | Decrease DEF 60% + Stun Target | 5/5 |

**Expected Damage:** 70-90M (manual)  
**Strengths:** ×1.6 Decrease DEF multiplier (better than Weaken) + fast base SPD (107) easier to tune  
**Weaknesses:** Stag Knight not owned (need to acquire), less tanky than Geomancer for stun target

---

#### **VARIANT 3: Mithrala (Auto-Safe)**

| Champion | SPD | Role | Importance |
|----------|-----|------|------------|
| Leonardo | 267 | Unkillable/Tank | 5/5 |
| **Mithrala** | 277 | Cleanse + Shield | 4/5 |
| Michelangelo | 270 | Join Attack DPS | 5/5 |
| Brogni | 274 | Block Debuffs | 5/5 |
| Geomancer | 263 | Weaken + HP Burn + Stun Target | 4/5 |

**Expected Damage:** 45-65M (manual), 40-60M (auto)  
**Strengths:** Cleanse recovers from Decrease SPD, 90-95% auto success rate  
**Weaknesses:** NO Buff Extension (Leonardo only 50% Unkillable uptime = 2 turns vs 3)

---

### **Alternative Champions**

**Owned Alternatives with Clan Boss Viability:**

| Champion | Role | Expected Damage | Importance | Why Use? |
|----------|------|-----------------|------------|----------|
| **Mithrala Lifebane** | Cleanse/Shield | 45-65M (manual), 40-60M (auto) | 4/5 | AUTO safety - A3 cleanse recovers from Decrease SPD |
| **Rector Drath** | Revive/Heal | 40-60M (auto) | 3/5 | AUTO safety - A3 Revive + Perfect Veil backup |
| **Bad-el-Kazar** | Poison/Heal | 50-70M | 2/5 | POISON variant - extends debuffs +1 turn, Continuous Heal |

**Note:** Mithrala and Rector replace Godseeker for auto-safe runs. Bad-el-Kazar replaces Geomancer for Poison damage alternative (if time to invest in Poison champions).

---

### **Ideal Pulls**

| Champion | Role | How to Get | Why Pull? | Speed Needed |
|----------|------|------------|-----------|--------------|
| **Venus** | Dec DEF + Weaken | Sacred/Void Shards | ×2.0 multiplier (both debuffs) = 90-120M damage | 263-280 |
| **Duchess Lilitu** | Buff Extension ×2 | Void Shards | 100% Unkillable uptime (extends +2 vs Godseeker +1) | 263-280 |
| **Warlord** | Block Damage | Void Shards | 100% damage immunity (3 turns Block Damage) | 263-280 |
| **Roshcard the Tower** | Block Damage | Ancient/Sacred | Unkillable alternative (Force affinity, replaces Leonardo) | 264-270 |
| **Turvold** | DEF Nuker | Ancient/Sacred | 30-50M solo damage (DEF scaling nuke) | 263-280 |

---

### **Affinity Considerations**

**Damage by Affinity:**

| Affinity | Expected Damage | Best Team | Notes |
|----------|----------------|-----------|-------|
| **Void** | 60-80M | Geomancer variant | No affinity issues, highest consistency |
| **Spirit** | 70-90M | Geomancer variant | Leonardo strong (+15% damage, Void vs Spirit) |
| **Force** | 45-60M | Alternate needed | Leonardo weak (-30% damage, Void vs Force) ⚠️ |
| **Magic** | 55-75M | Geomancer variant | Brogni weak (-30% shield scaling, Force vs Magic) |

---

### **Affinity Replacements (Owned Champions Only)**

**Force Boss (Leonardo Void = Weak):**
- **Issue:** Leonardo -30% damage (Void weak to Force)
- **Ideal replacement:** Roshcard the Tower (Force, Block Damage) - 264-270 SPD (replaces Leonardo as Unkillable provider)

**Magic Boss (Brogni Force = Weak):**
- **Issue:** Brogni shield reduced by weak hits (Force weak to Magic)
- **Solution 1:** Increase Brogni HP to 70k+ (compensate for reduced shield)
- **Solution 2:** Mithrala (Magic) replaces Brogni, but loses Block Debuffs
- **Ideal replacement:** Warlord (Void, Block Damage 3 turns) - 263-280 SPD

---

**END OF GUIDE - Version 2.1 CORRECTED**

**Changes from v2.0:**
- ✅ **FIXED:** Godseeker does NOT have C.RATE buff (removed incorrect Geomancer 94% C.RATE calculation)
- ✅ **FIXED:** Leonardo A1 counterattack = 4 hits (not 40), Michelangelo joins add ~2-4 hits
- ✅ **FIXED:** Damage expectations corrected to 60-80M manual (was incorrectly 109-132M)
- ✅ **FIXED:** Speed tune reordered: Geomancer SLOWEST (263 SPD) for stun target
- ✅ **VERIFIED:** All champion skills checked against JSON files
- ✅ **VERIFIED:** Leonardo A2 books: 5→3 turn CD (not 5→4)
- ✅ **ADDED:** Detailed champion mechanics explanations

**Next Steps:**
1. Build Leonardo to 267 SPD with 3,500+ DEF and 100% C.RATE
2. Build Geomancer to 263 SPD (slowest) with 250+ ACC and 100% C.RATE
3. Regear Michelangelo to 270 SPD with 100% C.RATE
4. Test manual rotation, verify Godseeker A2 timing (right after Leonardo A2)
5. Benchmark damage with 5 runs, report results

**Questions? Issues?** All mechanics verified from champion JSON files!
