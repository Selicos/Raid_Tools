...
You are to generate a complete champion log for Ithos in JSON format, using the following module templates as structure and guidance. Each module (0–13) is included below. For each, fill in the relevant information for Ithos. Output a single JSON object with each module as a key (e.g., "overview", "skills", "synergy", etc.).
...
---
Instructions:
- Fill in each section for Ithos using the module templates below.
- Output only the final JSON object.

---
After generating the JSON object, save it as `[champion].json` (e.g., `Ithos.json`) in the `Champions` directory under your project root.
If a file with the same name exists, overwrite it with the new data.
